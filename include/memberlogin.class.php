...

{
    ...
    function CheckLogin(&$loginuser, $loginpwd)
    {
        global $dsql;
        //检测用户名的合法性
        $rs = CheckUserID($loginuser,'用户名',FALSE);
    ...
    function CheckPhoneLogin(&$loginuser)
    {
        global $dsql;
        //matt=10 是管理员关连的前台帐号，为了安全起见，这个帐号只能从后台登录，不能直接从前台登录
        $row = $dsql->GetOne("SELECT mid,matt,pwd,logintime FROM `#@__member` WHERE phone LIKE '$loginuser' ");
        if(is_array($row)){
            //管理员帐号不允许从前台登录
            if($row['matt']==10) {
                return -2;
