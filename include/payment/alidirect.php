...
/**
 * 支付宝免签约即时到帐接口类 www.zfbjk.com
 */
class alidirect
{
    var $dsql;
    var $mid;
    var $return_url = "/plus/carbuyaction.php?dopost=return&code=alidirect";
    /**
     * 构造函数
     *
     * @access  public
    ...
    */
    function GetCode($order, $payment)
    {
        ...
        $button = '<form action="https://shenghuo.alipay.com/send/payment/fill.htm" method="post" name="form1" accept-charset="gb2312" onsubmit="document.charset=\'gb2312\';"><input name="optEmail" value="'.$payment['alipay_account'].'" type="hidden"><input name="payAmount" type="hidden" value="'.$order['price'].'"><input name="title" type="hidden" value="'.$order['out_trade_no'].'" /><input name="memo" type="hidden" value="付款后自动到帐，付款说明请勿自行修改" /><input type="submit" value="立即使用支付宝支付" /></form>';
    ...
    */
    function respond()
    {
        ...
        if(preg_match ("/S-P[0-9]+RN[0-9]/",$order_sn)) {
            ...
            elseif($row['money'] != $_POST['Money']) return $msg = "支付失败，支付金额与商品总价不相符!";
            $ordertype = "member";
            $product =    $row['product'];
            $pname= $row['pname'];
            $pid=$row['pid'];
