...
if(!isset($dopost) || empty($dopost)){
    ...
    */
    ...
    } else if ($do == 'clickout')
    {
        ...
        if(empty($rows['oid']))
        {
            $sql = "INSERT INTO `#@__shops_orders` (`oid`,`userid`,`cartcount`,`price`,`phone`,`email`,`state`,`ip`,`stime`,`pid`,`paytype`,`dprice`,`priceCount`)
            VALUES ('$OrdersId','$userid','$CartCount','$priceCount','$tel','$email','0','$ip','$stime','$pid','$paytype','$dprice','$lastpriceCount');";
            ...
            SET `cartcount`='$CartCount',`price`='$priceCount',`ip`='$ip',`phone`='$tel',`email`='$email',`stime`='$stime',pid='$pid',paytype='$paytype',dprice='$dprice',priceCount='$lastpriceCount'
