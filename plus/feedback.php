...
else if($action=='' || $action=='show')
{
	...
	if(empty($arcRow['aid']))
    {
        $querystring = "SELECT fb.*,mb.userid,mb.face as mface,mb.spacesta,mb.scores,mb.sex,fb.reply FROM `#@__feedback` fb
                 LEFT JOIN `#@__member` mb on mb.mid = fb.mid
                 WHERE fb.ischeck='1' $wquery ORDER BY fb.id desc";
    }else{
		$querystring = "SELECT fb.*,mb.userid,mb.face as mface,mb.spacesta,mb.scores,mb.sex,fb.reply FROM `#@__feedback` fb
                 LEFT JOIN `#@__member` mb on mb.mid = fb.mid
	...
	$dlist->SetParameter('reply',$reply);
