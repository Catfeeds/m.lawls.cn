...

{
...

    {
        ...
        /* $query = "SELECT id, tid, tidname, tid2, tid2name,content, uid, title, digest, reward, dateline, expiredtime,
                  solvetime, status, replies, lastanswer FROM `#@__ask`
                  WHERE uid='{$this->cfg_ml->M_ID}' ORDER BY lastanswer DESC"; */
        $query = "SELECT a.*,ak.content askcon,f.msg feedback FROM `#@__ask` as a left join `#@__askanswer` ak on a.id=ak.askid left join `#@__feedback` f on f.username=ak.username GROUP BY a.id  HAVING a.uid='{$this->cfg_ml->M_ID}' ORDER BY a.lastanswer DESC";
