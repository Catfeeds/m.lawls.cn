...
class question extends Control
{
    function question()
	{
		...
		{
            $gourl = $cfg_cmspath.'/ask/?ct=question&ac=ask_complete';
            ShowMsg('您尚未登录，请先登录！',$cfg_cmspath.'/member/login.php?gourl='.urlencode($gourl));
			exit;
		}
		//载入帮助
...

	{
        ...
        {
            $gourl = $cfg_cmspath.'/ask/?ct=question&ac=ask_complete';
            ShowMsg('请先登录，然后才能进行提问！',$cfg_cmspath.'/member/login.php?gourl='.urlencode($gourl));
            exit();
        }
		...
		if(preg_match("#7#",$GLOBALS['safe_gdopen'])){
            ...
            if(strtolower($data['vdcode']) != $svali || $svali=='')
            {
                ...
                ShowMsg('验证码错误！', "/ask/?ct=question&ac=ask_complete");
		...
		if($rs){
            ShowMsg('请不要重复发布同一问题,请耐心等待解答..', "/ask/?ct=question&ac=ask_complete");
    		exit; 
		}
		//保存问题
		...
		if($rs) 
		{
			...
			//if($do == 'add'){
				...
				$id =empty($maxid)? "" : intval($maxid);
				$reward =empty($data['reward'])? "" : intval($data['reward']);
				$buynum = ($buynum < 1) ? 1 : $buynum;
				$rs = $this->dsql->GetOne("SELECT id,channel,title FROM #@__archives WHERE id='$id'");
				if(!is_array($rs))
				{
					ShowMsg("该商品已不存在！","-1");
					exit();
				...
				$rows['buynum'] = $buynum;
				$rows['title']= "在线提问：".$data['tid2name'];
				$cart->addItem($id, $rows);
				//获得购物车内商品,返回数组
				$Items = $cart->getItems();
				if(empty($Items))
				{
					ShowMsg("您的购物车中没有商品！","/ask/?ct=question&ac=ask_complete");
					exit();
				}
				$OrdersId = preg_replace("#[^0-9a-z_\-]#i", "", $cart->OrdersId);        //本次记录的订单号
				//$rows['oid'] = $OrdersId;
			//}
			$query = "UPDATE `#@__ask` SET oid='$OrdersId' WHERE id='$maxid'";
			$this->dsql->ExecuteNoneQuery($query);
			header("location:/plus/carbuyaction.php");
    		exit; 
    	}else{
