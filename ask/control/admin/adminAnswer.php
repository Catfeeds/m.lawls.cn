 ...
 *
 **/
class adminAnswer extends Control
{
    ...
    function ac_index()
    {
		...
		//当前位置
		$nav = $GLOBALS['cfg_ask_position'].' <a href="/ask/?ct=browser&tid='.$question['tid'].'">'.$question['tidname'].'</a>';
		if($question['tid2'])
		{
			$nav .= ' '.$GLOBALS['cfg_ask_symbols'].' <a href="/ask/?ct=browser&tid2='.$question['tid2'].'">'.$question['tid2name'].'</a>';
...

	{
        ...
        /*if(!$cfg_ml->IsLogin())
        {
             ShowMsg("请先登录，然后才能进行提问！","/member/login.php");
            exit();
        }*/
	    $title = request('title', '');
...

	{
		...
		if($data['title'] == '')
		{
			ShowMsg('问题名称不能为空', "/ask/?ct=question&ac=ask_complete");
			exit;
		}else if(strlen($data['title']) > 200){
			ShowMsg('问题不能大于200字节', "/ask/?ct=question&ac=ask_complete");
			exit;
		}else if(strlen($data['title']) < 8){
			ShowMsg('问题不能小于8字节', "/ask/?ct=question&ac=ask_complete");
			exit;
		}
		//检查问题内容
		if(empty($data['content']))
		{
			ShowMsg('问题说明内容不能为空!', "/ask/?ct=question&ac=ask_complete");
			exit;
		}
		...
		if($ClassLevel1 < 1){
			ShowMsg('未指定栏目或栏目id不正确，请返回', "/ask/?ct=question&ac=ask_complete");
			exit;
		}
		$data['tid2name'] = $ClassLevel2;
		//计算过期时间
		...
		/*if($rs){
            ShowMsg('请不要重复发布同一问题,请耐心等待解答..', "?ct=adminAnswer&ac=ask_complete");
    		exit; 
		}*/
		//保存问题
		...
		if($rs) 
		{
    		...
    		ShowMsg('问题添加成功！', "/ask/admin.php?ct=issue");
    		exit; 
    	}else{
...

	{
...

		{
		    ...
		    ShowMsg('非法操作，请返回', "/ask/?ct=question&ac=ask_complete");
...

		{
...

    		{
    			...
    			ShowMsg('问题已经过期', "/ask/?ct=question&ac=ask_complete");

    			exit;

    		}else if($question['status'] >= 1){

    			ShowMsg('问题已经解决或者已过期,不能被修改!', "/ask/?ct=question&ac=ask_complete");
		...
		$this->SetTemplate('ask_adminedit.htm');
...

	{
...

        {
			...
			ShowMsg('问题修改成功！',"/ask/admin.php?ct=issue");
			exit;

		}else{

		    ShowMsg("编辑失败!","/ask/admin.php?ct=adminAnswer&askaid=".$data['askaid']);
...

	{
...

		{
			...
			if($rs){
    			...
    			ShowMsg('修改积分成功!',"/ask/admin.php?ct=adminAnswer&askaid=".$askaid);

    			exit();

    		}else{

    		    ShowMsg('修改积分失败!',"/ask/admin.php?ct=adminAnswer&askaid=".$askaid);
...

	{
...

		{
			...
			ShowMsg("设置成功！","/ask/admin.php?ct=adminAnswer&askaid=".$askaid);

			exit;

		}else{

		    ShowMsg("设置失败！","/ask/admin.php?ct=adminAnswer&askaid=".$askaid);
...

	{
		...
		$data['uid'] = $this->cfg_ml->M_ID;
		$askaid = $data['askaid'];
		$sql = "SELECT m.* FROM `#@__member` as m left join `#@__ask` as a on m.mid=a.uid  WHERE a.id='{$askaid}'";
		...
		ShowMsg('回答问题成功,如果未显示答案，请等待管理人员审核...',"/ask/admin.php?ct=issue");
...

	{
		...
		$this->SetTemplate('admin_reply.htm');
...

	{
...

		{
			...
			ShowMsg('您的回答已经修改成功！将返回问题页面',"/ask/admin.php?ct=answer");
...

	{
...

		{
			...
			ShowMsg("采纳成功!","/ask/admin.php?ct=adminAnswer&askaid=".$answer['askid']);
